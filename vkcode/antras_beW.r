...
rez.0 <- foreach(n.s=1:length(steb),.combine='rbind')%:%
          foreach(i=1:iter,.combine='rbind') %dopar% {
                ...
                n.o <- length(coef(mod))-omit
                if(no.of.coef>0&no.of.coef<=n.o){n.o <- no.of.coef}
                dat <- data.frame(that=coef(mod)[1:n.o],index=1:n.o)
                xtx <- t(yx[c.1:n,2:(c.1*m+1-omit)])%*%yx[c.1:n,2:(c.1*m+1-omit)]
                xtx <- xtx[1:n.o,1:n.o]
...
#                W <- solve(t(Ch))
                ...
                h0.try <- try({
                   ...
                   h0 <- t(resid(eq.u))%*%ginv(sd(resid(mod))^2*(diag(length(resid(eq.u)))-gr.h0%*%ginv(t(gr.h0)%*%gr.h0)%*%t(gr.h0))%*%ginv(xtx)%*%t(diag(length(resid(eq.u)))-gr.h0%*%ginv(t(gr.h0)%*%gr.h0)%*%t(gr.h0)))%*%resid(eq.u)
                    })
                ...
                h1.try <- try({
                    ...
                    h1 <- t(resid(eq.r))%*%ginv(sd(resid(mod))^2*(diag(length(resid(eq.r)))-gr.h1%*%ginv(t(gr.h1)%*%gr.h1)%*%t(gr.h1))%*%ginv(xtx)%*%t(diag(length(resid(eq.r)))-gr.h1%*%ginv(t(gr.h1)%*%gr.h1)%*%t(gr.h1)))%*%resid(eq.r)

#                    h1 <- t(e1-lamb)%*%(e1-lamb)
                })
                if(class(h1.try)=="try-error") h1 <- NA
                ifelse(!is.na(h0),
#                    {vec <- c(h0,h1,k.x,kmax,e1,1111,1112,omit,sol.meth.h0,sol.meth.h1,n,1e+10)

                    {vec <- c(h0,h1,k.x,kmax,coefficients(eq.u)[1:4],length(resid(eq.u)),length(resid(eq.u))-length(coef(eq.u)),omit,sol.meth.h0,sol.meth.h1,n,1e+10)
                    names(vec) <- c("H0","H1","IC","kmax","lambda.1","lambda.2","alpha","beta","nobs_eq.u","df_eq.u","omit.last","sol.meth.H0","sol.meth.H1","n","1e+10")
                    },
                    {vec <- c(h0,h1,k.x,kmax,c(1:6)*NA,omit,sol.meth.h0,sol.meth.h1,n,1e+10)
                    })
                    names(vec) <- c("H0","H1","IC","kmax","lambda.1","lambda.2","alpha","beta","nobs_eq.u","df_eq.u","omit.last","sol.meth.H0","sol.meth.H1","n","1e+10")
                    vec
       }
...
#          foreach(z.s=(m*c(dfmm[2]:dfmm[3])),.combine='rbind')%dopar{
...
#####################
#####################
s.file<-sprintf("/home/scratch/Virmantas/dat_midas/_II_k0%1.f_beW/H0_iter.%1.f_steb.%1.f_type.%s.%.2f_m.%1.f_ic.%1.f_omit.%1.f_power.%.2f_dal.interv.%1.f.Rdata",k0,iter,steb,type,rho.v,m,info.type,omit,laipsn,dal.interv)[1]

#save(iter,steb,type,rho.v,m,l.0,a.0,b.0,info.type,omit,sd.x,sd.y,rez.0,rez.1,rez.2,file=s.file)
